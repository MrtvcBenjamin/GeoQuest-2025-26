...
In der Diplomarbeit „GeoQuest“ habe ich die Projektmanagement-Aufgaben übernommen und zusätzlich technischen Support in der Entwicklung geleistet. Dazu zählten insbesondere die Koordination im Team, Termin- und Qualitätsmanagement, die Abstimmung mit Auftraggeber und Betreuung, die laufende Pflege der Projektdokumentation sowie die Unterstützung bei technischen Umsetzungs- und Integrationsfragen. Ziel meines Vorgehens war ein leichtgewichtiges, aber nachvollziehbares Projektmanagement, das für ein Schulprojekt praktikabel ist und gleichzeitig eine saubere Grundlage für Entscheidungen, Änderungen und Qualitätskontrolle schafft [@european_commission_pm2_2018].
...
Methodisch wurde dafür eine hybride Vorgehensweise gewählt. Als Arbeitsmodus wurden agile Prinzipien genutzt, um in kurzen Zyklen greifbare Zwischenstände zu erzeugen und Feedback früh zu berücksichtigen. Gleichzeitig wurde mit PM² ein strukturierender Rahmen verwendet, der insbesondere Rollen, Planungsartefakte, Änderungsbehandlung und Controlling abbildet. Diese Kombination ist für ein Schulprojekt mit realem Auftraggeber zweckmäßig, weil Anpassungsfähigkeit und Nachvollziehbarkeit gleichermaßen erforderlich sind [@beck_manifesto_2001] [@european_commission_pm2_2018].

### Iteratives Vorgehen und inkrementelle Umsetzung
...
### Projektorganisation, Rollen und Stakeholder
...
Die Zusammenarbeit wurde über kurze, regelmäßige Abstimmungen organisiert, in denen offene Punkte geklärt und Entscheidungen vorbereitet wurden. Aus Sicht des Projektmanagements war dabei wesentlich, dass Entscheidungen nicht nur informell getroffen, sondern nachvollziehbar dokumentiert und in die relevanten Projektartefakte übernommen wurden. Dadurch blieb für Auftraggeber und Betreuung transparent, auf welcher Grundlage Umfang, Prioritäten und technische Vorgehensweisen festgelegt wurden [@european_commission_pm2_2018].

### Terminplanung über Fixtermine und Meilensteine
...
### Dokumentenlenkung und Nachvollziehbarkeit

Ein wiederkehrendes Problem in kleinen Softwareprojekten ist, dass Wissen über Entscheidungen und Annahmen nur in Gesprächen existiert und später nicht mehr reproduzierbar ist. Um dem entgegenzuwirken, wurde eine klare Dokumentenstruktur als projektinterne Referenz etabliert. Das Projekthandbuch diente dabei als zentrale Quelle für Rahmenbedingungen, Rollen und Prozesse, der Meilensteinplan als strukturierende Termin- und Lieferobjektplanung, die Anwendungsfälle als präzise Beschreibung der funktionalen Abläufe und Randfälle sowie die Statusberichte als laufende Verdichtung von Ist-Zustand, Abweichungen und Maßnahmen. In Summe entsteht dadurch eine durchgängige Nachvollziehbarkeit, bei der Anforderungen, Entscheidungen, Umsetzungsschritte und Qualitätssicherung nicht isoliert stehen, sondern konsistent aufeinander verweisen und Änderungen kontrolliert eingearbeitet werden [@european_commission_pm2_2018].
...
### Use-Case-Struktur für Abläufe, Randfälle und Systemzustände
...
### Akzeptanzkriterien mit Given/When/Then (BDD/Gherkin-Stil)
...
### Offene Punkte und Entscheidungsfindung (Conversation Points)
...
### Projektcontrolling, Statusberichte und Qualitätssicherung
...
### Konfigurationsmanagement und Toolchain-Konsistenz

Im Projekt zeigte sich, dass technische Planbarkeit nicht nur von Quellcode und Anforderungen abhängt, sondern auch von der Entwicklungsumgebung. Gerade in Flutter-Projekten ist die Menge auflösbarer Paketversionen eng an die verwendete Flutter- und Dart-SDK-Version gekoppelt, sodass unterschiedliche lokale Toolchain-Stände schnell zu abweichenden Ergebnissen beim Auflösen von Abhängigkeiten führen können. Aus Projektmanagement-Sicht ist dies relevant, weil es direkt auf Reproduzierbarkeit, Build-Stabilität und damit auf Termin- und Qualitätsziele wirkt. In vergleichbaren Projekten wird deshalb häufig eine per Projekt festgelegte Flutter-Version verwendet, etwa über Flutter Version Management (FVM), um konsistente Builds und nachvollziehbare Upgrades sicherzustellen. In GeoQuest war diese Erfahrung ein wesentlicher Grund, warum Abhängigkeitsauflösung und Toolchain-Pflege im praktischen Teil einen eigenen Schwerpunkt bilden [@dart_pubspec_2025] [@flutter_upgrade_2026] [@fvm_overview_2026].
...
### Grundlagen: Was sind Dependencies und warum können sie kollidieren?
...
### Wie Versionsangaben funktionieren: SemVer, Constraints und Caret-Syntax
...
### Warum „version solving failed“ passiert: PubGrub und das „eine Version pro Paket“-Prinzip
...
### Typische Konfliktarten in Flutter (und warum sie in Schulprojekten so häufig sind)

#### Direkte Versionskonflikte
...
#### Transitive Konflikte („Dependency Hell“)
...
#### SDK-Constraints: Dart-/Flutter-Version passt nicht
...
#### Flutter „Pinned Packages“ (besonders relevant in unserem Projekt)
...
Ein typischer Konfliktablauf ließ sich dabei als wiederkehrendes Muster beobachten. Das Flutter SDK pinnt bestimmte Paketversionen, während einzelne Plugins oder Libraries Mindestversionen oder enge Versionsbereiche definieren, die diese gepinnten Versionen nicht mehr einschließen. Der Version-Solver findet dann keine konsistente Kombination und bricht mit einer nicht auflösbaren Constraint-Kette ab. In der Praxis führte das zu zwei stabilen Lösungswegen: Entweder wurde die lokale Flutter- und Dart-Version auf einen Stand gebracht, der mit den neueren Constraints kompatibel ist, oder die betroffene Dependency wurde bewusst auf eine kompatible Generation zurückgesetzt beziehungsweise die Constraint so erweitert, dass die gepinnte Version wieder innerhalb des erlaubten Bereichs liegt [@dart_flutter_pinned_packages_2026] [@dart_pubgrub_solver_2026] [@flutter_upgrade_2026] [@dart_pub_dependencies_2026].

#### Konflikte in nativen Abhängigkeiten (Gradle und CocoaPods)
Flutter-Plugins binden nicht nur Dart-Pakete ein, sondern bringen häufig auch plattformspezifische Abhängigkeiten für Android und iOS mit. Unter Android erfolgt die Auflösung über Gradle, unter iOS über CocoaPods. Beide Werkzeuge konstruieren ebenfalls einen Abhängigkeitsgraphen und müssen Versionskonflikte lösen, wodurch Konflikte auftreten können, obwohl die Dart-Abhängigkeiten bereits konsistent auflösbar sind. Flutter weist darauf hin, dass Platform-spezifische Abhängigkeiten auf Gradle-Modulen und CocoaPods in ähnlicher Weise behandelt werden, sodass Konfliktursachen und Lösungen auch auf dieser Ebene zu betrachten sind. Für die Praxis bedeutet das, dass bei Build-Problemen nicht nur die pubspec.yaml, sondern bei Bedarf auch die native Auflösung in Gradle beziehungsweise die Podfile- und Podspec-Konstellation analysiert werden muss [@flutter_using_packages_2026] [@android_gradle_dependency_resolution_2026] [@gradle_dependency_constraints_conflicts_2026] [@cocoapods_faq_dependency_resolution_2026] [@cocoapods_podfile_reference_2026].

### Vorgehensweise bei der Diagnose (wie ich Konflikte systematisch zerlegt habe)
...
### Lösungsstrategien (Beheben von Dependency Conflicts)

#### Flutter-/Dart-SDK aktualisieren (sauberste Lösung bei „SDK zu alt“)
...
#### Dependency-Versionen anpassen (typisch bei „Dependency zu neu“)
...
#### Constraints weiten statt unnötig fixieren
...
#### dependency_overrides (kurzfristige Notlösung)
...
#### Cache- und Build-Artefakte als Störfaktor
Nicht jeder Fehler bei der Auflösung oder beim Build ist ein echter Versionskonflikt. In der Praxis können auch lokale Caches und Build-Artefakte dazu führen, dass ein Projekt trotz unveränderter Dependencies nicht mehr reproduzierbar baut. Pub verwaltet einen System-Cache für heruntergeladene Pakete, der in seltenen Fällen problematische Zustände enthalten kann und dann bewusst bereinigt werden muss. Zusätzlich können lokale Build-Verzeichnisse und generierte Tooling-Artefakte veraltet oder inkonsistent sein, wodurch ein vollständiger Neuaufbau erforderlich wird. Dart und Flutter stellen dafür entsprechende Reinigungsmechanismen bereit, etwa über das Leeren des Pub-Caches sowie über das Entfernen lokaler Build- und Tooling-Verzeichnisse, um eine saubere Ausgangslage für die erneute Auflösung und den Neuaufbau herzustellen [@dart_pub_cache_2025] [@flutter_cli_reference_2026].

#### Lockfile bewusst nutzen (Reproduzierbarkeit im Team)
...
### Prävention (Verhindern statt Reparieren)

#### Toolchain vereinheitlichen (Flutter-Version im Team fixieren)
...
#### Updates geplant durchführen (kleine Schritte statt große Sprünge)
...
#### CI/Build stabilisieren (Lockfile erzwingen)
...
#### Update-Governance und automatisierte Aktualisierung
Ein Teil der Konfliktentstehung ist nicht rein technisch, sondern prozessbedingt. Wenn Updates ungeplant und nebenbei erfolgen, kann eine einzelne neue Paketversion die Auflösung destabilisieren, obwohl der restliche Entwicklungsstand unverändert bleibt. Stabiler ist es, Updates als geplanten Vorgang zu behandeln, etwa durch feste Update-Fenster, gezielte Tests nach Upgrades und Pull-Request-basierte Aktualisierungen, um Auswirkungen früh sichtbar zu machen. Werkzeuge wie Dependabot unterstützen dieses Vorgehen, indem sie Abhängigkeitsupdates automatisiert als Pull Requests bereitstellen und damit eine kontrollierte, nachvollziehbare Update-Pipeline ermöglichen [@github_dependabot_version_updates_2026].

### Bezug zur Diplomarbeit: Warum bei uns fast immer „SDK vs. Dependency“ der Kern war
